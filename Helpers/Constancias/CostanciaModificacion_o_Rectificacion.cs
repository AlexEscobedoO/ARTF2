...
namespace artf_MVC.Helper.Constancias
{
    public class CostanciaCancelacion
    {
         public IActionResult Generar(Equiuni equiuni, string Title)
        {
            ...
            if (Title == "RECTIFICACIÓN")
            {                
                equiRec = equiuni.NumacuofsolNavigatorNavigation.Rectrves?.FirstOrDefault();
            }
            else
            {
                equiMod = equiuni.NumacuofsolNavigatorNavigation.Modrves?.FirstOrDefault();
            }
            using (var pdfStream = new MemoryStream())
            {
                ...
                {
                    //numeroTable3_1 = equi.IdmodequiNavigation.Idmod;
                    ...
                    numeroTable3_1 = equiMod.Idmod;
                    Document = equiuni.NumacuofsolNavigatorNavigation.Numacuofsol;
                    FechaDocumento = equiMod.Fechamod.ToString();
                    Descripcion = equiMod.Desmod;
                    Observaciones = equiMod.Obsmod;
                    NoOficio = equiuni.NumacuofsolNavigatorNavigation.Numacuofsol;
                    FechaOficio = equiuni.NumacuofsolNavigatorNavigation.Fecapsol.ToString();
                }
                else
                {
                    //numeroTable3_1 = equi.Idrect; 
                    //Document = "Convenio Número F/16943-7";
                    //FechaDocumento = equi.Fecharect.ToString();
                    //Descripcion = equi.Desrect;
                    //Observaciones = equi.Obsrect;
                    //NoOficio = equiuni.NumacuofsolNavigatorNavigation.Numacuofsol;
                    //FechaOficio = equiuni.NumacuofsolNavigatorNavigation.Fecapsol.ToString();
                    numeroTable3_1 = equiRec.Idrect;
                    Document = "Convenio Número F/16943-7";
                    FechaDocumento = equiRec.Fecharect.ToString();
                    Descripcion = equiRec.Desrect;
                    Observaciones = equiRec.Obsrect;
                    NoOficio = equiuni.NumacuofsolNavigatorNavigation.Numacuofsol;
                    FechaOficio = equiuni.NumacuofsolNavigatorNavigation.Fecapsol.ToString();
            ...
            static PdfPTable CreateCustomSubTable4(string Marca, string Uso)
            {
                ...
                {
                    // Tercer rectángulo con texto "TRABAJO"
                    PdfPCell rectangleCellC = new PdfPCell(new Phrase("MIXTO", new Font(Font.FontFamily.HELVETICA, 7f)));
